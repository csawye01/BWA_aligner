...
def uploadparts_BWA():
    ...
    if form.validate_on_submit():
        ...
        form.genbank_file.data.save('app/static/temp/' + GB_filename)
        R1_filename = secure_filename(form.R1_file.data.filename)
        form.R1_file.data.save('app/static/temp/' + R1_filename)
        R2_filename = secure_filename(form.R2_file.data.filename)
        form.R2_file.data.save('app/static/temp/' + R2_filename)

        # use BWA aligner to index the GenBank file and align the paired reads
        ref_fasta = 'app/static/temp/' + GB_filename
        read_1 = 'app/static/temp/' + R1_filename
        read_2 = 'app/static/temp/' + R2_filename
        cmd_index = "bwa index " + ref_fasta
        cmd_align = "bwa mem " + ref_fasta + " " + read_1 + " " + read_2 + \
                    " | samtools sort -O BAM -o app/static/temp/output.bam -"
        os.system(cmd_index)
        os.system(cmd_align)

        # create consensus sequence from bam file
        cdm_sortbam = "samtools sort app/static/temp/output.bam  -o output_sorted.bam"
        cmd_bam2fasta = "samtools mpileup -uf " + ref_fasta + " app/static/temp/output_sorted.bam | bcftools call -c| vcfutils.pl vcf2fq > app/static/temp/cons.fq"
        os.system(cdm_sortbam)
        os.system(cmd_bam2fasta)

        SeqIO.convert("app/static/temp/cons.fq", "fastq", "app/static/temp/cons_fasta.fasta", "fasta")
        ref_record = SeqIO.read(ref_fasta, "fasta")
        RefSeq = ref_record.seq
        plas_record = SeqIO.read('app/static/temp/cons_fasta.fasta', "fasta")
        ...
        with open("app/static/temp/combined_seqs.fasta", 'w') as output_handle:
            SeqIO.write(records, output_handle, "fasta")

        clustalomega_cline = ClustalOmegaCommandline(infile="app/static/temp/combined_seqs.fasta",
                                                     outfile="app/static/temp/aligned_seqs.fasta",
                                                     verbose=True, auto=True, force=True)
        clustalomega_cline()

        fasta_sequences = SeqIO.parse(open("app/static/temp/aligned_seqs.fasta"), 'fasta')
        ...
        return redirect(url_for('app.Aligned_Seqs'))

    return render_template('index.html', form=UploadReads, message=message)
